...
const  AllCategorize = () => {
    ...
    const createPathName = (title: string) => {
    ...
    const renderCategoryIcon = (icon: string) => {
        if (isImageIcon(icon)) {
            return <img src={resolveImageUrl(icon)} alt="分类图标" style={{ width: 28, height: 28, objectFit: 'cover', borderRadius: 6 }} />
        }
    ...
    const onfinish = async () => {
        ...
        const data:{ color: string; introduce: string;categoryTitle: string; icon: string,pathName:string } = {
            categoryTitle: categorie.trim(),
            icon:categorie_icon,
            color: normalizeColor(categorie_color),
            introduce:introduce,
            pathName: form.getFieldsValue().pathName || createPathName(categorie)
        }
        try {
    ...
    const handleOk = async () => {
        try {
            ...
            if (isEdit !== 0) {
                const update = {
                    categoryTitle: values.categorie.trim(),
                    introduce: values.introduce,
                    icon: values.categorie_icon,
                    color: normalizeColor(values.categorie_color),
                    pathName: values.pathName || createPathName(values.categorie)
                }
    ...
    const columns: TableProps<CategoriesType>['columns'] = [
        ...
        },
        {
            ...
            key: 'key',
            align: "center",
            render: (icon) => renderCategoryIcon(icon)
        },
        {
            title: '文章数量',
            key: 'key',
    ...
    return <>
        ...
        >
            <Form {...formItemLayout} variant="filled" style={{ maxWidth: 600 }} form={form} onFinish={onfinish}>
                <Form.Item label="分类名称" name="categorie" rules={[{ required: true, message: '请输入分类名称' }]}>
                    <Input/>
                </Form.Item>
                ...
                <Form.Item label="分类图标" name="categorie_icon"
                           rules={[{ required: true, message: 'Please input!' }]}
                >
                    <Input placeholder="可填图标类名或 uploads/xxxx.png"/>
                </Form.Item>

                <Form.Item label="颜色" name="categorie_color" initialValue="#000000" >
