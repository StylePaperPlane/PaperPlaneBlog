...
## 使用
```bash
cd && mkdir -p PaperPlane/core && cd $_
mkdir -p db_init
wget -O db_init/init.sql https://cdn.jsdelivr.net/gh/LinMoQC/PaperPlane-Core@master/db_init/init.sql
wget https://cdn.jsdelivr.net/gh/LinMoQC/PaperPlane-Core@master/docker-compose.yml
docker-compose up -d
...
-p 9999:9999 \
-d --restart=always \
-e "MINIO_ROOT_USER=paperplane" \
-e "MINIO_ROOT_PASSWORD=paperplane" \
