...
(
    ...
    icon           varchar(300)           not null comment 'icon',
...
(
    ...
    folder_name varchar(50) default '默认文件夹' not null comment '文件夹名称'
)
    comment '图库表';

create table if not exists image_folders
(
    folder_key int unsigned auto_increment comment '唯一标识'
        primary key,
    folder_name varchar(50) not null comment '文件夹名称',
    constraint image_folders_folder_name
        unique (folder_name)
)
    comment '图片文件夹表';

INSERT IGNORE INTO image_folders (folder_name)
VALUES ('默认文件夹');
